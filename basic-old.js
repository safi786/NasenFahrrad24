...
async function takepicture() {
  ...
  let eye1 = { x: parseInt(width/2.3), y: parseInt(height/5), sizeX: 40, sizeY: 55, src: "./Png to svgs/Eye1-borderless.svg",};
  let eye2 = {...eye1, x: parseInt(width/1.5),  src: "./Png to svgs/Eye2-borderless.svg"};
  let nose = { x: parseInt(width/1.86), y: parseInt(height/4.5), sizeX: 45, sizeY: 280, src: "./Png to svgs/Nose-borderless.svg", };
...
function addCanvasEvents() {
  ...
  canvas.addEventListener("mousemove", function (e) {
    ...
    e.stopPropagation();
    if(isDragging) {
      shapesArr[selectedShape].x = e.offsetX - (shapesArr[selectedShape].sizeX/2);
      shapesArr[selectedShape].y = e.offsetY - (shapesArr[selectedShape].sizeY/2);
      drawConsole();
    }
    return;
  });
